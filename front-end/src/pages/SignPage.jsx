...
useEffect(() => {
  ...
  return (
    <form onSubmit={handleSubmit(handleForm)} className=" bg-gradient-to-r from-blue-500 to-purple-600 h-screen">
      ...
      <input defaultValue={'Dalbert'} {...register('name')} className='inputForm' type='text' placeholder="username"/>
      <input defaultValue={1234} {...register('password')} className="inputForm" type='password' placeholder="password"/>
  ...
  <button onClick={()=>navigate('/signup')} className='text-center text-xs text-blue-700 font-mono'>SIGN UP</button>
      </div>
     </div>
    </form>
  )
